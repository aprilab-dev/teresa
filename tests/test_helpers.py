...
polygons = ["POLYGON((118.4939 36.8231,117.446 35.7963,118.7813 \
...
def test_find_bursts(polygons):
    slc_img = create_slcimage(date="20210822", sourcedir=os.path.join(os.path.dirname(__file__), "aux/s1/sourcedir"))
    slc_img.crop(aoi=polygons)
    assert slc_img.IW1["first_burst_index"] == 3
    assert slc_img.IW3["last_burst_index"] == 18
    assert len(slc_img.source) == 2
